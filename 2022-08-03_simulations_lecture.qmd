...
-   90% **<citation needed>** of simulations are the same!
...
Split is programmed by `parent = <pop>` argument:
...
Simply take a model object and run it using a built-in simulation engine in R:

```{r, eval = FALSE}
ts1 <- msprime(model1, sequence_length = 100e6, recombination_rate = 1e-8)
```
...
::: fragment
```{r}
ts_samples(ts1) %>% count(pop)
```
:::

For downstream population genetic analyses, individuals can be referred to by their symbolic names.
