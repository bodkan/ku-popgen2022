...
<center>![](images/slendr_logo.png){width="30%"}</center>
...
Now that we have defined our populations and their histories:
...
We have our compiled model, how do we simulate data from it?
...
This interface links to Python methods implemented in [_tskit_](https://tskit.dev/tskit/docs/stable/python-api.html).
...
<!-- ```{r} -->
<!-- for (N_test in seq(1000, 20000, by = 1000)) { -->
<!--   pop <- population("pop", N = N_test, time = 100000) -->
<!--   model <- compile_model(pop, generation_time = 1, direction = "backward") -->
...
<!--   new_afs <- ts_afs(ts, list(samples), polarised = TRUE) -->
<!--   lines(new_afs, type = "b", col = "black", add = TRUE) -->
<!-- } -->
<!-- ``` -->
...
Use _slendr_ to simulate history of this species and guess the likely value of squirrel's $N_e$ given the observed AFS?
...
2. Find the $N_e$ value that will give the closest AFS to the one you got from Hestu. Use whatever method you're comfortable with based on your programming experience:
    ...
    a) Plot different simulated AFS vectors along with the observed AFS and just eye-ball it.
  
    b) Simulate AFS across a grid of $N_e$ values and find the closest matching one (maybe use [mean-squared error](https://en.wikipedia.org/wiki/Mean_squared_error)?)
  
    c) Run a mini-Approximate Bayesian Computation, using the Oracle's range of [10000, 30000] as a uniform prior.


## Exercise #2 -- simulations on a grid (solution b)
...
plot(Ne_grid, errors)
abline(v = Ne_grid[which.min(errors)], color = "green")
abline(v = TRUE_NE, color = "purple")
...
## Exercise #2 -- ABC inference (solution c)
...
Of course, this scenario was extremely basic!

But we could easily include other demographic events, more complex history, gene flow...

Speaking of more complex models and analyses...
...
<!-- plot_model(model1, proportions = TRUE) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- ts_diversity(ts1, samples, mode = "branch") %>% arrange(diversity) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- ts1 <- ts_mutate(ts1, mutation_rate = 1e-8) -->
...
<!-- cowplot::plot_grid( -->
<!--   plot_model(model2) + ggtitle("Normal time scale") + title, -->
<!--   plot_model(model2, log = TRUE, proportions = TRUE) + ggtitle("Time on the log-scale") + title -->
<!-- ) -->
<!-- ``` -->

<!-- ## Parameter values -->

<!-- -   "CH" outgroup population at time 7 Mya ($N_e$ = 10k) -->
...
<!--   model1, times = seq(from = 7000, to = 4000, by = -100), -->
<!--   list(x1, 5), list(x2, 5) -->
<!-- ) -->
...
<!-- ```{r} -->
<!-- ts1 <- msprime(model1, sequence_length = 50e6, recombination_rate = 1e-8, -->
<!--                samples = schedule) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- x_inds <- ts_samples(ts1) %>% filter(pop %in% c("x1", "x2")) -->

<!-- x_names <- x_inds$name -->

<!-- f4_result<- map_dfr(x_names, function(w) ts_f4(ts1, W = w, X = "c_1", Y = "b_1" , Z = "o_1")) -->
...
<!-- ```{r} -->
<!-- ts1_small <- ts_simplify(ts1, simplify_to = c("o_1", "a_1", "b_1", "c_1", "x1_1", "x2_1")) -->
