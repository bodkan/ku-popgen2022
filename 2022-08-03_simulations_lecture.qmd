...
gf <- gene_flow(from = b, to = x1, start = 5100, end = 6000, rate = 0.3)
...
#| fig-align: center
ggplot(x_inds) +
  geom_rect(aes(xmin = 5100, xmax = 6000, ymin = -Inf, ymax = Inf), alpha = 0.75, fill = "gray") +
...
- one "European" every 1000 years between 40 kya and present-day
- one "Altai" (70 ky old) and one "Vindija" individual (40 ky old)
...
<!-- ```{r} -->
<!-- pdf("trees.pdf") -->
<!-- for (i in sort(sample(seq_len(ts_small$num_trees), 200))) { -->
<!--   plot(ts_phylo(ts_small, i - 1), main = i - 1)  -->
<!-- } -->
<!-- dev.off() -->
<!-- ``` -->


<center>**Only the coalescent nodes of genealogies involving these samples will be retained.**</center>


## Extracting a tree #1

```{r}
tree <- ts_phylo(ts_small, 0)
```
...
<center>**Maybe this is introgression?</center>


## Extracting a tree #1908

```{r}
tree <- ts_phylo(ts_small, 1909)
```
...
<!-- -   Choose a statistic whose value "true value" you know from the literature (divergence? diversity? f4-statistic? ancestry proportion? divergence?) -->
<!-- - Try to replicate the statistic in the simulation -->
