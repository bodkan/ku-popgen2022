...
gf <- gene_flow(from = b, to = x1, start = 5100, end = 5500, rate = 0.3)
...
#| fig-align: center
ggplot(x_inds) +
  geom_rect(aes(xmin = 5100, xmax = 5500, ymin = -Inf, ymax = Inf), alpha = 0.75, fill = "gray") +
...
```{r}
tree <- ts_phylo(ts_small, 0, quiet = TRUE)
```
...
```{r}
tree <- ts_phylo(ts_small, 1909, quiet = TRUE)
```
