...
<!-- #| code-fold: true -->
<!-- #| code-summary: "Show solution" -->
<!-- simulate_future_afs <- function(Ne_start, resize_factor) { -->
<!--   Ne_end <- as.integer(Ne_start * resize_factor) -->

<!--   if (Ne_start == Ne_end) -->
<!--     model <- "constant" -->
<!--   else if (Ne_start > Ne_end) -->
<!--     model <- "contraction" -->
<!--   else -->
<!--     model <- "expansion" -->

<!--   pop <- population("squirrels", N = Ne_start, time = 1) -->

<!--   if (model != "constant") -->
<!--     pop <- resize(pop, N = Ne_end, time = 5000, how = "step") -->

<!--   ts <- -->
<!--     compile_model(pop, generation_time = 1, simulation_length = 10000) %>% -->
<!--     msprime(sequence_length = 50e6, recombination_rate = 1e-8) %>% -->
<!--     ts_mutate(mutation_rate = 1e-8) -->

<!--   samples <- ts_samples(ts) %>% sample_n(50) %>% pull(name) -->
<!--   afs <- ts_afs(ts, list(samples), polarised = TRUE) -->
...
<!-- afs1 <- simulate_future_afs(Ne_start = 6543, resize_factor = 1) -->
<!-- afs2 <- simulate_future_afs(Ne_start = 6543, resize_factor = 1/3) -->
<!-- afs3 <- simulate_future_afs(Ne_start = 6543, resize_factor = 3) -->

<!-- afs <- rbind(afs1, afs2, afs3) -->

<!-- ggplot(afs, aes(bin, freq, color = model)) + -->
