...
Let's make the algorithm even more minimal by focusing on an evolution of a single mutation across time.
...
## Tree-sequence tables ([link](https://tskit.dev/tutorials/tables_and_editing.html))
...
library(ape)
plot(t); nodelabels(t$node.label)
```

::: {.column width="50%"}
```{r}
# get nodes of the tree
nodes <- ts_nodes(t_phylo)$node_id
ts_table(ts_tiny, "nodes") %>% filter(node_id %in% nodes)
ts_table(ts_tiny, "edges") %>%
  filter(parent %in% nodes | child %in% nodes,
         left == t_tskit$interval$left)
```
:::
