...
::: aside
Documentation on [tables](https://tskit.dev/tutorials/tables_and_editing.html) from the tskit project.
:::
...
```{r}
#| code-fold: true
ts_nodes(t_phylo) %>% as_tibble()
ts_edges(t_phylo) %>% as_tibble()
```
:::
...
```{r}
#| eval: false
ts <- msprime(model, sequence_length = 1e6, recombination_rate = 1e-8, output = "<path>")
```
...
What about pairwise genetic divergence between all populations (`ts_divergence()`)? Do your results recapitulate phylogenetic relationships? How about `ts_f3()` or `ts_fst()`?
:::
