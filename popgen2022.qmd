...
4.   Learning concepts in population genetics
...
## Learning concepts in population genetics
...
RStudio sometimes interferes with Python setup that we need for simulation.

To avoid the problem, go to `Tools` -> `Global Options` and make sure you have the following setting:

<center>![](images/rstudio_setting.png)</center>
...
```{r}
#| code-line-numbers: "|16|17|18|19|20|15"
pop1 <- population("pop1", N = 1000, time = 1)
...
```{r}
#| code-fold: true
ts_nodes(t_phylo) %>% as_tibble() %>% head(3)
ts_edges(t_phylo) %>% as_tibble() %>% head(3)
```
:::
...
```{r}
observed_afs <- c(2520, 1449, 855, 622, 530, 446, 365, 334, 349, 244, 264, 218, 
...
Help the Oracle predict the future shape of the AFS of the squirrels after 10000 generations, assuming starting $N_e$ = 6543? Species #1 will remain constant, species #2 will expand 3X, species #3 will get 3X smaller.
